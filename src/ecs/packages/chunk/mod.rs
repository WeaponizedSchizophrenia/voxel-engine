use bevy_ecs::{entity::Entity, query::Changed, schedule::IntoSystemConfigs as _, system::{Commands, Query, Res}};
use nalgebra::Vector2;

use crate::ecs::{components::{Chunk, RenderDescriptor}, schedules::Update};
...
impl Package for ChunkPackage {
    fn initialize(&mut self, app: &mut crate::application::Application) {
        ...
        app.add_systems(Update, chunk_mesher_system.after(generator::generate_chunk_data));
    }
}
...
pub fn chunk_mesher_system(
    ...
    for (entity, chunk) in chunks.iter() {
        for (_voxel, mesh) in chunk.build_mesh_new(&render_context) {
            commands.entity(entity)
                .insert((
                    voxel_render_descriptor.clone(),
                    mesh
                ));
        }
