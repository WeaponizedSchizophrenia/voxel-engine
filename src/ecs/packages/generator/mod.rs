...
pub fn generate_chunk_data(mut query: Query<&mut Chunk, Added<Chunk>>, generator: Res<Generator>) {
    query.par_iter_mut().for_each(|mut chunk| {
        (0..chunk::CHUNK_LENGTH)
            .for_each(|x| {
                (0..chunk::CHUNK_LENGTH)
                    .for_each(|z| {
                        let world_pos = vector![x as f32, z as f32] + (chunk.get_index() * chunk::CHUNK_LENGTH as i32).map(|c| c as f32);
                        ...
                        for y in 0..height {
                            *chunk.sample_mut((x, y, z))
                                .unwrap() = Some(Voxel { id: 0 });
                        }
                    });
            });
    });
